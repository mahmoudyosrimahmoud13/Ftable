...
def main():
    ...
    frame1 = Frame(main, bg="#CBCBCB", width=680,
                   height=100).place(x=10, y=180)

    user = username_value.get()
    admin_name = f"admin: {user}"
    admin = Label(main, frame1, text=admin_name, font="Bahnschrift 13",
                  bg="#CBCBCB", fg="#F8485E").place(x=10, y=190)

    mycursor.execute(F"SELECT id FROM admin WHERE username = '{user}'")
    id = mycursor.fetchone()

    id_num = F"id: {id[0]}"
    id = Label(main, frame1, text=id_num, font="Bahnschrift 13",
               bg="#CBCBCB", fg="#F8485E").place(x=10, y=235)

    day_name = strftime("Day: %A")
    day = Label(main, frame1, text=day_name, font="Bahnschrift 13",
                bg="#CBCBCB", fg="#F8485E").place(x=500, y=190)

    date_num = strftime("Date: %D")
    date = Label(main, frame1, text=date_num, font="Bahnschrift 13",
                 bg="#CBCBCB", fg="#F8485E").place(x=500, y=235)

    frame2 = Frame(main, bg="#D2D2D2", width=680,
                   height=240).place(x=10, y=290)

    agent = Label(main, frame2, text="Agent Data", bg="#D2D2D2",
                  font="Bahnschrift 18", fg="#00C1D4").place(x=10, y=300)

    agent_name_value = StringVar()
    id_value = IntVar()

    agent_name = Label(main, text="Agent username", background="#D2D2D2",
                       foreground="#F8485E", font="Bahnschrift 13", )
    ...
    id_label = Label(main, frame2, text="Agent ID",
                     bg="#D2D2D2", font="Bahnschrift 13", fg="#F8485E")
    id_label.place(x=10, y=400)
    id_entry = ttk.Entry(main, textvariable=id_value)
    id_entry.place(x=150, y=403)

    error = Label(main, frame2, text='', bg="#D2D2D2",
                  font="Bahnschrift 13", fg="red")
...
# Enter function
    ...
    def enter():
        ...
        try:
            ...
            if id == int(mycursor.fetchone()[0]):
                ...
                # Day list:
                day_list_value = ['Saturday', 'Sunday', 'Monday',
                ...
                launch = Label(main, frame2, text="Launch: ",
                               bg="#D2D2D2", font="Bahnschrift 13", fg="#F8485E")
                launch.place(x=10, y=360)

                meal_1 = Label(main, frame2, text="Mael", bg="#D2D2D2",
                               font="Bahnschrift 13 bold", fg="#512D6D")
                meal_1.place(x=10, y=390)
                list_l_m = ttk.Combobox(main, value=list_luanch_meals)
                list_l_m.place(x=75, y=395)
                list_l_m.set("Meal")

                drink_1 = Label(main, frame2, text="Drink", bg="#D2D2D2",
                                font="Bahnschrift 13 bold", fg="#512D6D")
                ...
                break_fast = Label(main, frame2, text="Breakfast: ",
                                   bg="#D2D2D2", font="Bahnschrift 13", fg="#F8485E")
                break_fast.place(x=10, y=430)

                meal_2 = Label(main, frame2, text="Mael", bg="#D2D2D2",
                               font="Bahnschrift 13 bold", fg="#512D6D")
                ...
                drink_2 = Label(main, frame2, text="Drink", bg="#D2D2D2",
                                font="Bahnschrift 13 bold", fg="#512D6D")
                ...
                day = Label(main, frame2, text="Day: ", bg="#D2D2D2",
                            font="Bahnschrift 13", fg="#512D6D")
        ...
        def confirm():
            try:
                ...
                data = (user, day, breakfast_meal,
                        breakfast_drink, lunch_meal, lunch_drink)
    ...
    quit_btn_main = ttk.Button(
        main, text="Quit", command=quit_main).place(x=600, y=567)

    info = ttk.Button(main, text="User info",
                      command=user_info).place(x=20, y=540)

    search = ttk.Button(main, text="Search",
                        command=search_win).place(x=20, y=567)
...
def add():
    ...
    label = Label(add, text="Adding a new agent", fg="#00C1D4",
                  font="Bauhaus93 16 bold", padx=60, bg=color)
    label.place(x=50, y=10)
    agent_user_label = Label(
        add, text="user: ", fg="#F8485E", font="Bauhaus93 13 bold", bg=color)
    agent_user_label.place(x=10, y=45)
    agent_user_entry = ttk.Entry(add, textvariable=agent_user)
    agent_user_entry.place(x=70, y=45)

    agent_phone_label = Label(
        add, text="phone: ", fg="#F8485E", font="Bauhaus93 13 bold", bg=color)
...
def user_info():
    ...
    # agent information:
    agent_info_tab = Frame(user_info, width=500, height=500,
                           background="#cccccc", bg=color)
    ...
    # admin information:
    admin_info_tab = Frame(user_info, width=500, height=500,
                           background="#cccccc", bg=color)
    columns = ['firstname', 'lastname',
               'username', 'phone', 'emergency', 'gender']
    ...
    mycursor.execute(
        "SELECT firstname,lastname,username,phone,emergency,gender FROM admin")
    id = 0
    ...
    scale_admin = ttk.Scrollbar(admin_info_tab, orient='vertical',
                                command=table_admin_info.yview).place(x=0, y=0, height=500)

    # data:
    data_tab = Frame(user_info, width=500, height=500,
                     background="#cccccc", bg=color)
    columns = ['username', 'day', 'breakfast_meal',
               'breakfast_drink', 'lunch_meal', 'lunch_drink']
    ...
    mycursor.execute(
        "SELECT username,day,breakfast_meal,breakfast_drink,lunch_meal,lunch_drink FROM data")
    id = 0
    ...
    scale_data = ttk.Scrollbar(
        data_tab, orient='vertical', command=table_data.yview).place(x=0, y=0, height=500)

    # Menu:
    menue_tab = Frame(user_info, width=500, height=500,
                      background="#cccccc", bg=color)
    ...
    # history_log
    history_log = Frame(user_info, width=500, height=500,
                        background="#cccccc", bg=color)

    # login
    log_in_label = Label(history_log, text="Login history:", background="#cccccc",
                         foreground="#00C1D4", font="Bauhaus93 16 bold", bg=color).pack()
    ...
    for log in mycursor.fetchall():
        if str(log[0]) != 'None':
            ...
            id += 1
    table_login.pack()
    scale_login = ttk.Scrollbar(history_log, orient='vertical',
                                command=table_login.yview).place(x=25, y=30, height=230)

    # logout
    log_out_label = Label(history_log, text="Logout history:", background="#cccccc",
                          foreground="#00C1D4", font="Bauhaus93 16 bold", bg=color).pack()
    ...
    for log in mycursor.fetchall():
        if str(log[0]) != 'None':
            ...
            id += 1
    table_logout.pack()
    scale_logout = ttk.Scrollbar(history_log, orient='vertical',
                                 command=table_logout.yview).place(x=25, y=290, height=180)
...
def search_win():
    ...
    # admin search tab:
    admin_tab = Frame(search, width=500, height=500,
                      background="#cccccc", bg=color)
    tabs_search.add(admin_tab, text='Admin')
    # variable:
    search_box_admin_value = StringVar()

    search_box_entry_admin = ttk.Entry(
        admin_tab, textvariable=search_box_admin_value, width=75)
    ...
    def search_commdand_admin():
        ...
        mycursor.execute(
            f"SELECT firstname,lastname,username,phone,emergency,gender FROM admin WHERE username LIKE '%{str(search_box_admin_value.get())}%'")
        id = 0
        for data in mycursor.fetchall():
    ...
    search_button_admin = ttk.Button(
        admin_tab, cursor="hand2", image=search_img, command=search_commdand_admin)
    search_button_admin.pack()

    # agent search tab
    agent_tab = Frame(search, width=500, height=500,
                      background="#cccccc", bg=color)
    ...
    search_box_entry_agent = ttk.Entry(
        agent_tab, textvariable=search_box_agent_value, width=75)
    ...
    def search_commdand_agent():
        ...
        mycursor.execute(
            f"SELECT id,username,phone FROM agent WHERE username LIKE '%{str(search_box_agent_value.get())}%'")
        id = 0
        for agent_info in mycursor.fetchall():
            id_agent = str(agent_info[0])
            table_agent_info.insert(parent='', index='end', iid=id, text='',
    ...
    search_button_agent = ttk.Button(
        agent_tab, cursor="hand2", image=search_img, command=search_commdand_agent)
    search_button_agent.pack()

    # data search tab
    data_tab = Frame(search, width=500, height=500,
                     background="#cccccc", bg=color)
    tabs_search.add(data_tab, text='Data')
    # variable:
    search_box_data_value = StringVar()

    search_box_data_entry_data = ttk.Entry(
        data_tab, textvariable=search_box_data_value, width=75)
    ...
    def search_commdand_data():
        ...
        mycursor.execute(
            f"SELECT username,day,breakfast_meal,breakfast_drink,lunch_meal,lunch_drink FROM data WHERE username LIKE '%{str(search_box_data_value.get())}%'")
        id = 0
        for data in mycursor.fetchall():
            table_data.insert(parent='', index='end', iid=id, text='',
                              values=(data[0], data[1], data[2], data[3], data[4], data[5]))
    ...
    search_button = ttk.Button(
        data_tab, cursor="hand2", image=search_img, command=search_commdand_data, width=30)
    search_button.pack()
...
def sign_in():
    ...
    Label(sign_in, text="Sign in", font="Berlin 20 bold",
          fg="#00C1D4", bg=color).grid(row=0, column=3)

    firstname = Label(sign_in, text="First name", fg="#F8485E",
                      font="Bauhaus93 13 ", bg=color).grid(row=1, column=3)
    fristname_entry = ttk.Entry(
        sign_in, textvariable=firstname_value).grid(row=1, column=4)

    lastname = Label(sign_in, text="last name", fg="#F8485E",
                     font="Bauhaus93 13 ", bg=color).grid(row=3, column=3)
    lastname_entry = ttk.Entry(
        sign_in, textvariable=lastname_value).grid(row=3, column=4)

    user_name = Label(sign_in, text="Username", fg="#F8485E",
                      font="Bauhaus93 13 ", bg=color).grid(row=5, column=3)
    user_name_entry = ttk.Entry(
        sign_in, textvariable=user_name_value).grid(row=5, column=4)

    emergency = Label(sign_in, text="Emergency number", fg="#F8485E",
                      font="Bauhaus93 13 ", bg=color).grid(row=7, column=3)
    fristname_entry = ttk.Entry(
        sign_in, textvariable=emergency_value).grid(row=7, column=4)

    phone_number = Label(sign_in, text="phone number", fg="#F8485E",
                         font="Bauhaus93 13 ", bg=color).grid(row=9, column=3)
    fristname_entry = ttk.Entry(
        sign_in, textvariable=phone_number_value).grid(row=9, column=4)

    password_sign = Label(sign_in, text="Password:", fg="#F8485E",
                          font="Bauhaus93 13 ", bg=color).grid(row=11, column=3)
    password_entry_sign = ttk.Entry(
        sign_in, textvariable=password_sign_value, show="*").grid(row=11, column=4)

    password_c_sign = Label(sign_in, text="Confirm your password:", fg="#F8485E", font="Bauhaus93 13 ", bg=color).grid(row=13,
                                                                                                                       column=3)
    password_c_entry_sign = ttk.Entry(
        sign_in, textvariable=password_c_sign_value, show="*").grid(row=13, column=4)

    gender_l = Label(sign_in, text="Gender:", fg="#F8485E",
                     font="Bauhaus93 13 ", bg=color).grid(row=14, column=3)
    male = ttk.Radiobutton(sign_in, text="Male", value='Male',
                           variable=gender_value).place(x=180, y=215)
    female = ttk.Radiobutton(
        sign_in, text="Female", value='Female', variable=gender_value).place(x=240, y=215)
    chekbox = ttk.Checkbutton(
        sign_in, text="Remember me?", variable=get_value).grid(row=15, column=4)
    ...
    def submet():
        ...
        if password_c_sign_value.get() != password_sign_value.get():
            ...
            data = (firstname_value.get(), lastname_value.get(), emergency_value.get(), phone_number_value.get(),
                    user_name_value.get(), password_c_sign_value.get(), gender_value.get())
            mycursor.execute(sql, data)
            db.commit()
            sign_in.destroy()
            done()

    submit = ttk.Button(sign_in, text="submit", cursor="hand2",
                        command=submet).grid(row=16, column=4)
...
def feedback_window():
    ...
    photo = Label(feedback, image=feedback_logo,
                  bg=color, width=50, height=50).pack()
    Label(feedback, text="Help us improve our product",
          font="Berlin 15 ", foreground="#00C1D4", background=color).pack()
    ...
    send_btn = ttk.Button(feedback, text='Send',
                          command=send).place(x=120, y=200)
    feedback.mainloop()
...
Label(root, text="Sign in", font="Berlin 20 bold",
      foreground="#00C1D4").pack(pady=17)
login_img = Label(root, image=login_file, width=100, height=100).pack(pady=15)
username = Label(root, text="Username:", fg="#F8485E",
                 font="Bauhaus93 13 bold").pack(pady=10)
username_entry = ttk.Entry(root, textvariable=username_value)
username_entry.pack()
password_entry_root = Label(
    root, text="Password:", fg="#F8485E", font="Bauhaus93 13 bold").pack(pady=10)
...
def login():
    ...
    try:
        mycursor.execute(
            F"SELECT password FROM admin WHERE username = '{user}'")
        ...
        if password_db == password:
            ...
            mycursor.execute(
                f"INSERT INTO history_log (login) VALUES ('{user} Log in at {date} in Time: {time}')")
            db.commit()
            main()
        else:
...
chekbtn = ttk.Checkbutton(root, text="Remember me?",
                          variable=get_value, offvalue="off", onvalue="on")
chekbtn.pack()
...
def quit_app():
    try:
        user = username_value.get()
        mycursor.execute(
            F"SELECT username FROM admin WHERE username = '{user}'")
        date = str(now.date())
        time = str(now.time())

        if user == ''.join(mycursor.fetchone()):
            mycursor.execute(
                f"INSERT INTO history_log (logout) VALUES ('{user} Log out at {date} in Time: {time}')")
            db.commit()
            root.destroy()
    except:
        root.destroy()
...
information_btn = ttk.Button(
    root, image=i_icon, command=information, cursor="hand2").place(x=265, y=20)
